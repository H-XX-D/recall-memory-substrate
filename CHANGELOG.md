...
- Hooks: the portable Python lifecycle hook now has a structured `PostToolUse` mode, turn-id-partitioned private state, successful-exit verification markers, `last_assistant_message` support, bounded state cleanup, and structured transcript fallback. Prompt compilation ignores non-zero CLI results, inner SessionStart timeouts fit below the outer hook timeout, and Stop is silent on a pass and loop-guarded on a continuation.
- Claude sync: registers all five supported modes (`SessionStart`, `UserPromptSubmit`, `UserPromptExpansion`, `PostToolUse`, and `Stop`) and refreshes Recall-owned handlers individually, preserving unrelated siblings in mixed matcher groups. The optional Node write-gate handlers are owned and toggled without duplication. Config, backups, hook state, and installed assets use private atomic writes.
- Codex sync: installs the current packaged, MCP-first Recall skill at `$CODEX_HOME/skills/recall/SKILL.md`, the generated `/recall` prompt at `$CODEX_HOME/prompts/recall.md`, the managed MCP and AGENTS surfaces, and the reviewed portable `SessionStart`, `UserPromptSubmit`, `PostToolUse`, and `Stop` hooks. This deliberately supersedes 0.12.0's no-Codex-skill design with a fresh v5 contract; it does not revive the obsolete skill tree. Sync refreshes only Recall-owned content with private atomic writes and `.bak` backups, preserves unrelated handlers, reports inline-hook conflicts, removes the retired duplicate AGENTS marker, and leaves trust review to Codex's `/hooks` UI. Status covers skill and prompt currentness, the hook asset, and all four registrations.
- MCP routing: after explicit `--db`, explicit `--project`, and `RECALL_DB`, `recall-mcp` now selects the deepest registered project containing its process working directory before falling back to home. The server remains single-store for its lifetime, and routed writes carry the selected project scope.
- Tests: the portable hook suite is now part of `npm test`, with regression coverage for transcript spoofing, failed verification, per-turn state, handler-level ownership, write-gate toggling, Codex skill and prompt installation, Codex event compatibility, custom-handler preservation, private file modes, `CODEX_HOME`, and MCP cwd routing.
- Migration/install: pre-0.6 `archived` nodes now remain non-active as v5 `annexed` cells instead of being reactivated, and the npm lockfile's root metadata now matches package version 0.12.1 for clean `npm ci` installs.
...
- Programs (fix): `runProgramCell` now reads a program's own persisted state (its `props.lastRun` baseline and `runCount`) from the store rather than the passed cell object, so a caller reusing a stale in-memory program across runs still sees its prior run. Production paths (the operator tick, CLI, MCP) always passed a fresh cell or key and were unaffected; this closes the footgun for direct programmatic callers, where `watch`/`drift`/`trend` would otherwise silently never trip.
- Write guidance: `programSuggestions` now suggests a `trend` program alongside `allocate`, `watch`, and `quorum`. When a write records a numeric `value` onto a topic that already carries a series of readings (>= 3), it suggests a trend program measuring `value`, which flags a sustained rise or fall across operator passes (distinct from `watch`'s single-step move on effective confidence).
...
- The credentials gate flags instead of blocking: a write whose text matches a credential pattern is accepted, automatically marked `sensitivity: secret`, and answered with a warning naming the token type and field; a `public` write carrying personal data is downgraded to `private` the same way. Nothing is rejected for content anymore. The overview documents the safety practices this relies on (stores and archives stay out of version control; store pointers to secrets, not values). The python helper's refusal of `sensitivity: secret` proposals is removed.
