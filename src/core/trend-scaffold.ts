...
export interface TrendScaffoldOptions {
  objective: string;
  maxMovers?: number;
  measure?: "effective_confidence" | "member_count";
  window?: number;
  delta?: number;
  streak?: number;
}
...
export function buildTrendScaffold(store: RecallStore, options: TrendScaffoldOptions): TrendScaffold {
  ...
  const directive =
    ...
    "MOVER: measure is what the trend tracks across runs, either effective_confidence (the bundle's mean live trust) or member_count. " +
